...
class AdminController extends BaseController
{
    ...
    public function dashboard()
    {
        ...
        $data['total_admins'] = (new AdminModel())->countAllResults();
        $data['total_roles'] = (new AdminRoleModel())->countAllResults();
        
        $data['active_categories'] = (new CategoryModel())->where('status', 'active')->countAllResults();
        $data['inactive_categories'] = (new CategoryModel())->where('status', 'inactive')->countAllResults();

        $data['floor_images'] = (new CategoryImageModel())->countAllResults();
        ...
        try {
            ...
            if ($img = $this->request->getFile('logo')) {
                if ($img->isValid() && !$img->hasMoved()) {
                    ...
                    $path = getcwd() . '/public/assets/img';
                    $img->move($path, $newName, true);
                    $imageUpload = new File("{$path}/{$newName}");
